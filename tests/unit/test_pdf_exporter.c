...
static void test_pdf_null_args(void) {
    TEST_ASSERT_EQUAL(GG_ERR_INVALID_ARG, gg_pdf_export(NULL, scale_repo, course_repo));
    TEST_ASSERT_EQUAL(GG_ERR_INVALID_ARG, gg_pdf_export(test_pdf_path, NULL, course_repo));
    TEST_ASSERT_EQUAL(GG_ERR_INVALID_ARG, gg_pdf_export(test_pdf_path, scale_repo, NULL));
}

static void test_pdf_export_success(void) {
    GGStatus status = gg_pdf_export(test_pdf_path, scale_repo, course_repo);
    TEST_ASSERT_EQUAL(GG_OK, status);

    FILE *fp = fopen(test_pdf_path, "rb");
    TEST_ASSERT_NOT_NULL(fp);
    char header[10];
    size_t read = fread(header, 1, 5, fp);
    TEST_ASSERT_EQUAL_UINT(5, read);
    header[5] = '\0';
    TEST_ASSERT_EQUAL_STRING("%PDF-", header);
    fclose(fp);
}

static void test_pdf_export_multipage(void) {
    for (int i = 0; i < 50; i++) {
        GGCourseEntry entry;
        memset(&entry, 0, sizeof(entry));
        snprintf(entry.semester_label, sizeof(entry.semester_label), "Year %d Sem 1", (i / 10) + 1);
        snprintf(entry.course_label, sizeof(entry.course_label), "CRS%03d", i);
        entry.credit_unit = 3;
        snprintf(entry.grade_symbol, sizeof(entry.grade_symbol), "A");
        entry.entry_date = 1700000000 + i;
        int64_t out_id = 0;
        GGStatus status = course_repo->insert_course(course_repo->context, &entry, &out_id);
        TEST_ASSERT_EQUAL(GG_OK, status);
    }
    ...
    long size = ftell(fp);
    fclose(fp);
    TEST_ASSERT_TRUE(size > 500);
}

int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_pdf_null_args);
    RUN_TEST(test_pdf_export_success);
    RUN_TEST(test_pdf_export_multipage);
    return UNITY_END();
}
